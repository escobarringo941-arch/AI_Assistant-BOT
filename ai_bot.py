...
TARGET_CHANNEL_ID = 1526384339670270012     # #ai-agency-chat — هنا غير AI كيهضر
WELCOME_CHANNEL_ID = 1524957892925456545     # #welcome — ترحيب الأعضاء الجداد
SERVER_NAME = "GGMW9"
...
MOD_LOGS_CHANNEL_ID = 1526470164235681832  # #mod-logs — هنا غير كاين logs كاملين
VERIFY_CHANNEL_ID = 1526481352264781854    # #verify
RULES_CHANNEL_ID = 1526474691789721700     # #rules

# الأدوار
UNVERIFIED_ROLE_ID = 1526452828267085915   # @Unverified
MEMBER_ROLE_ID = 1526451890399739934       # @Member
MUTED_ROLE_ID = 1526468718534590574        # @Muted
...
def get_system_prompt(user_gender="unknown"):
    ...
    base_prompt += '   - إلا سؤال علمي → جاوب بدقة وقول المصدر\n\n'
...
async def log_action(guild, title: str, description: str, color: discord.Color):
    """سجل الإجراءات فـ mod-logs فقط — ما فـ general أبداً"""
...
async def on_member_join(member):
    """ترحيب + Auto-Role (@Unverified) — الترحيب فـ welcome channel فقط"""
    ...
    # 2. يرحبو فـ welcome channel فقط (ما فـ general أبداً)
...
async def on_member_remove(member):
    """سجل خروج الأعضاء — فـ mod-logs فقط"""
    await log_action(
        member.guild,
        "👋 عضو خرج",
        f"**المستخدم:** {member.mention} ({member.name})\n"
...
async def on_raw_reaction_add(payload):
    """Verification: ملي يكليك ✅ — Log فـ mod-logs فقط"""
...
async def on_message_delete(message):
    """سجل حذف الرسائل — فـ mod-logs فقط"""
...
async def on_message_edit(before, after):
    """سجل تعديل الرسائل — فـ mod-logs فقط"""
...
async def auto_info():
    ...
    if news_channel:
        news_prompt = 'عطيني خبر تقني أو عالمي أو ثقافي جديد وممتع. اكتبو بالدارجة المغربية. ضيف مصدر إلا كان ممكن. كن مختصر (4-5 سطور فقط).'
    ...
    if games_channel:
        games_prompt = 'عطيني خبار جديدة على ألعاب الفيديو (release جديد، update، trailer...). اكتبو بالدارجة المغربية. قول شنو هو اللعبة واش جديد فيها. كن مختصر (4-5 سطور فقط).'
    ...
    if movies_channel:
        movies_prompt = 'عطيني فيلم جديد (رعب، أكشن، كوميديا، دراما...) مع: 1. السمية ديال الفيلم 2. النوع (Genre) 3. ملخص قصير وممتع 4. واش يستاهل التفرج ولا لا. اكتب بالدارجة المغربية. كن مختصر (5-6 سطور).'
    ...
    if anime_channel:
        anime_prompt = 'عطيني أنمي جديد (ولا حلقة جديدة ديال أنمي مشهور) مع: 1. السمية ديال الأنمي 2. النوع (shonen، isekai، romance...) 3. ملخص قصير وممتع 4. واش يستاهل التفرج. اكتب بالدارجة المغربية. كن مختصر (5-6 سطور).'
    ...
    if music_channel:
        music_prompt = 'عطيني خبار موسيقية جديدة: 1. سمية الأغنية والفنان 2. النوع (rap، pop، r&b، rock...) 3. شنو جديد فـ الأغنية 4. واش تستاهل التسمع. اكتب بالدارجة المغربية. كن مختصر (4-5 سطور).'
